...
The NA values are making it very hard to work with this tables. Replace the NA with the mean of minimum of all strains : 0.000153042615

```{r}
#replace all NA values with zero
merged_L2fc_a <- merged_L2fc_a %>% replace(is.na(.), 0.000153042615)
```
...
zscore_table <- select(merged_L2fc_a, 16:29)
...
geneExp_matrix <- as.matrix(zscore_table[,1:14])
geneExp_matrix
```
