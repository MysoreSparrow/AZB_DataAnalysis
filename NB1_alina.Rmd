...
downgenetable <- data.frame(f4$symbol)
#print(downgenetable)
summary(downgenetable, maxsum = 20)
```

```{r}
downgene.df <- as.data.frame(downgenetable)
count(downgene.df)
...
upgenetable <- data.frame(f3$symbol)
#view(upgenetable)
summary(upgenetable, maxsum = 30)

```


```{r}
upgene.df <- as.data.frame(upgenetable)
count(upgene.df)
upgene.df
```
...
```{r}
library(data.table)
setnames(downgenetable, "Gene_names")
setnames(upgenetable, "Gene_names")
```

```{r}
Row1<-as.vector(as.matrix( downgenetable[,1]) )
#downgene.df
print(Row1)
```
...
### Common genes between both tables!

```{r}
#fsetdiff(setDT(downgenetable), setDT(upgenetable), all = TRUE)
#intersecting_genes<- fintersect(downgenetable, upgenetable, all = F)
