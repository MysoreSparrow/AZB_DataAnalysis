...
```{r}
dds_ER <- estimateSizeFactors(dds_ER)

df <- bind_rows(
  as_data_frame(log2(counts(dds_ER, normalized=TRUE)[, 1:4]+1)) %>%
...
distance_plot <- pheatmap(sampleDistMatrix,
         ...
         main = "Sample-to-Sample Euclidean Distance - Epithelial Response",
         col = colors,
...
```{r}
poisd <- PoissonDistance(t(counts(dds_ER))) # raw counts or unnormalised data
samplePoisDistMatrix <- as.matrix( poisd$dd )
rownames(samplePoisDistMatrix) <- dds_ER$Samples
colnames(samplePoisDistMatrix) <- dds_ER$Samples
```
...
```{r}
percentVar <- round(100 * attr(pcaData, "percentVar"), digits = 4)
```
...
Reconfirminhg the PCA values with another transformation.
...
head(pcaData_rld)
percentVar_rld  <- round(100 * attr(pcaData_rld, "percentVar"), digits = 4)
...
## PCA Plot for different groupings VST Data
...
percentVar.PCAdata2 <- round(100 * attr(PCAdata2, "percentVar"), digits = 4)
...
percentVar.PCAdata3 <- round(100 * attr(PCAdata3, "percentVar"), digits = 4)
...
```{r}
PCAdata8 = plotPCA(vsd, intgroup = c("Samples","ER_microcolonies"), returnData = TRUE)
head(PCAdata8)
```

```{r}
percentVar.PCAdata8 <- round(100 * attr(PCAdata3, "percentVar"))

PCAplot_ERMC <- ggplot(PCAdata8, aes(x = PC1, y = PC2, 
                                   color = ER_microcolonies, 
                                   label= Samples)) +
  geom_point(size =2) +
  xlab(paste0("PC1: ", percentVar.PCAdata8[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar.PCAdata8[2], "% variance")) +
  coord_fixed() +
  scale_y_continuous(breaks=seq(-12, 12, 2))+
  scale_x_continuous(breaks=seq(-20, 20, 2))+
  ggtitle("PCA: EpithelialResponse_Microcolonies")+ 
  theme(legend.position="bottom") + 
  theme(aspect.ratio=25/75) +
  geom_text(size = 4, hjust=0, vjust=0)#+
  #scale_colour_manual(values = color_values)
...
PCAplot_ERMC
```
### PCA for Phylogentic Lineage
```{r}
pcadata9 <- plotPCA(vsd, intgroup = c("Samples","phylogenomic_lineage"), 
                    returnData = TRUE)
head(pcadata9)
```
```{r}
percentVar.PCAdata9 <- round(100 * attr(pcadata9, "percentVar"))

PCAplot9 <- ggplot(pcadata9, aes(x = PC1, y = PC2, 
                                   color = phylogenomic_lineage, 
                                   label= Samples)) +
  geom_point(size =2) +
  xlab(paste0("PC1: ", percentVar.PCAdata9[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar.PCAdata9[2], "% variance")) +
  coord_fixed() +
  scale_y_continuous(breaks=seq(-12, 12, 2))+
  scale_x_continuous(breaks=seq(-20, 20, 2))+
  ggtitle("PCA: phylogenomic_lineage")+ 
  theme(legend.position="bottom") + 
  theme(aspect.ratio=25/75) +
  geom_text(size = 4, hjust=0, vjust=0)#+
  #scale_colour_manual(values = color_values)
...
PCAplot9
```
### PCA for Phylogentic Lineage and Epithelial Response
```{r}
pcadata10 <- plotPCA(vsd, intgroup = c("Samples","Epithelial_response",
                                      "phylogenomic_lineage"), returnData = TRUE)
head(pcadata10)
```

```{r}
percentVar.PCAdata10 <- round(100 * attr(pcadata10, "percentVar"))

PCAplot10 <- ggplot(pcadata10, aes(x = PC1, y = PC2, 
                                   color = phylogenomic_lineage, 
                                   shape = Epithelial_response,
                                   label= Samples)) +
  geom_point(size =2) +
  xlab(paste0("PC1: ", percentVar.PCAdata10[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar.PCAdata10[2], "% variance")) +
  coord_fixed() +
  scale_y_continuous(breaks=seq(-12, 12, 2))+
  scale_x_continuous(breaks=seq(-20, 20, 2))+
  ggtitle("PCA: Epithelial_response & Phylogenomic Lineage")+ 
  theme(legend.position="bottom") + 
  theme(aspect.ratio=25/75) +
  geom_text(size = 4, hjust=0, vjust=0)#+
  #scale_colour_manual(values = color_values)
...
### PCA for Epithelial Response and Clinical Outcome
```{r}
...
PCAplot4
```
### PCA for Epithelial Response and Microcolonies
```{r}
...
PCAplot5
```
### PCA for Clinical Outcome and Microcolonies
```{r}
...
PCAplot6 <- ggplot(pcadata6, aes(x = PC1, y = PC2, 
  ...
  scale_y_continuous(breaks=seq(-12, 12, 2))+
  scale_x_continuous(breaks=seq(-20, 20, 2))+
  ggtitle("PCA: Clinical Outcome and Microcolonies")+ 
  theme(legend.position="bottom") + 
  theme(aspect.ratio=25/75) +
  geom_text(size = 4, hjust=0, vjust=0)#+
  #scale_colour_manual(values = color_values)
...
### PCA for Epithelial response, Clinical Outcome and Microcolonies
```{r}
...
### PCA for Phylogroup
```{r}
pcadata11 <- plotPCA(vsd, intgroup = c("Samples","phylogroup"), returnData = TRUE)
head(pcadata11)

```

```{r}
percentVar.PCAdata11 <- round(100 * attr(pcadata11, "percentVar"))

PCAplot11 <- ggplot(pcadata11, aes(x = PC1, y = PC2, 
                                 color = phylogroup, 
                                 label= Samples)) +
  geom_point(size =2) +
  xlab(paste0("PC1: ", percentVar.PCAdata11[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar.PCAdata11[2], "% variance")) +
  coord_fixed() +
  scale_y_continuous(breaks=seq(-12, 12, 2))+
  scale_x_continuous(breaks=seq(-20, 20, 2))+
  ggtitle("PCA: phylogroup")+ 
  theme(legend.position="bottom") + 
  theme(aspect.ratio=25/75) +
  geom_text(size = 4, hjust=0, vjust=0)#+
  #scale_colour_manual(values = color_values)
...
```{r}
pcadata12 <- plotPCA(vsd, intgroup = c("Samples","Intimin_Type"), returnData = TRUE)
head(pcadata12)

```

```{r}
percentVar.PCAdata12 <- round(100 * attr(pcadata12, "percentVar"))

PCAplot12 <- ggplot(pcadata12, aes(x = PC1, y = PC2, 
                                 color = Intimin_Type, 
                                 label= Samples)) +
  geom_point(size =2) +
  xlab(paste0("PC1: ", percentVar.PCAdata12[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar.PCAdata12[2], "% variance")) +
  coord_fixed() +
  scale_y_continuous(breaks=seq(-12, 12, 2))+
  scale_x_continuous(breaks=seq(-20, 20, 2))+
  ggtitle("PCA: Intimin_Type")+ 
  theme(legend.position="bottom") + 
  theme(aspect.ratio=25/75) +
  geom_text(size = 4, hjust=0, vjust=0)#+
  #scale_colour_manual(values = color_values)
...
```{r}
dds2_ER <- DESeq(dds_ER, minReplicatesForReplace=Inf)
res2_ER <- results(dds2_ER, cooksCutoff=FALSE, independentFiltering=FALSE)
...
```{r}
res2_ER.df <- as.data.frame(res2_ER) # convert the results table to a df
head(res2_ER.df)
```
...
resultsNames(dds2_ER)
```

```{r}
plotMA_res2_ER<- plotMA(res2_ER, ylim = c(-2, 2))
```


```{r}
res3_ER <- lfcShrink(dds2, coef="Epithelial_response_LowInducer_vs_HighInducer", type="apeglm")
```
```{r}
plotMA_res3ER <-plotMA(res3_ER, ylim = c(-2, 2))
```

### Histogram of p-values

```{r}
hist(res2_ER$pvalue, breaks = 50, col = "grey50", border = "blue")
```
 Further Filtering: baseMean > 1
```{r}
hist(res2_ER$pvalue[res2_ER$baseMean > 1], breaks = 50, col = "grey50", border = "blue")
...
```{r}
res2_ER.df$symbol <- mapIds(org.Mm.eg.db,
                     keys = rownames(dds_ER),
                    column = "SYMBOL",
                    keytype="ENSEMBL",
                     multiVals="first")
res2_ER.df$entrez <- mapIds(org.Mm.eg.db,
                     keys = rownames(dds_ER),
                    column = "ENTREZID",
                    keytype="ENSEMBL",
                     multiVals="first")
...
```{r}
nrow(res2_ER.df)
res3_ER.df <- res2_ER.df %>% filter(!is.na(symbol) & !is.na(entrez))
nrow(res3_ER.df)
```
...
```{r}
resOrdered_ER <- res3_ER.df[order(res3_ER.df$pvalue),]
head(resOrdered_ER)
```

```{r}
write.csv(as.data.frame(resOrdered_ER), file = "~/R/Alina_RNAseq/Epithelial_response_LowInducer_vs_HighInducer_results_ER_DifferentialGenes_gendergenesremoved.csv")
```
...
```{r}
select <- order(rowMeans(counts(dds2_ER,normalized=TRUE)),
                decreasing=TRUE)[1:20]
df_ER <- as.data.frame(colData(dds2_ER)[,c("Epithelial_response","Samples")])
```

```{r}
pheatmap(assay(vsd)[select,], 
         cluster_cols= TRUE,
         annotation_col= df_ER,
         color = heat.colors,
         show_rownames = FALSE)
```
...
```{r}
ntd_ER <- normTransform(dds2_ER)
meanSdPlot(assay(ntd_ER))
```
...
```{r}
p1 <- EnhancedVolcano(res3_ER.df,
    lab = res3_ER.df$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
...
```{r}
res3_ER.df$diffexpressed <- "NS"
# if log2Foldchange > 1.0 and pvalue < 0.05, set as "UP"
res3_ER.df$diffexpressed[res3_ER.df$log2FoldChange > 1.0 & res3_ER.df$pvalue < 0.05] <- "UP"
# if log2Foldchange < 1.0 and pvalue < 0.05, set as "UP"
res3_ER.df$diffexpressed[res3_ER.df$log2FoldChange < -1.0 & res3_ER.df$pvalue < 0.05] <- "DOWN"

# Create a new column "delabel" to de, that will contain the name of genes differentially expressed (NA in case they are not)
res3_ER.df$delabel <- NA
res3_ER.df$delabel[res3_ER.df$diffexpressed != "NS"] <- res3_ER.df$symbol[res3_ER.df$diffexpressed != "NS"]
```
...
DE_volcanoplot1 <- ggplot(res3_ER.df, aes(x=log2FoldChange , y = -log10(pvalue),
...
DE_volcanoplot <- ggplot(res3_ER.df, aes(x=log2FoldChange , y = -log10(pvalue),
...
```{r}
ggplot(res3_ER.df, aes(x=log2FoldChange , y = log10(baseMean),
...
```{r}
sigs_ER.df <- res3_ER.df[(abs(res3_ER.df$log2FoldChange)>1) & (res3_ER.df$pvalue< 0.05),]
head(sigs.df)
```
...
```{r}
sigs_ER.df <- filter(sigs_ER.df, symbol != "Xist", symbol !="Jpx", symbol !="Ftx", symbol !="Tsx", symbol != "Cnbp2" )
```
...
write.csv(sigs_ER.df ,"~/R/Alina_RNAseq/SignificantGenes_ER_Gendergenesremoved_thresholdimposed.csv")
```
...
```{r}
sigs_ER.UP.df <- sigs_ER.df[(sigs_ER.df$log2FoldChange)>1, ] #UP Regulation Table
sigs_ER.DOWN.df <- sigs_ER.df[(sigs.df$log2FoldChange)< -1, ]#DOWN Regulation Table
```
...
```{r}
GO_UPReg_results <- enrichGO(gene = UPgene_names,
                       OrgDb = "org.Mm.eg.db",
                       keyType = "ENSEMBL",
                       ont = "BP",
                       pAdjustMethod = "BH",
...
```{r}
GO_fit <- plot(barplot(GO_results, showCategory = 9))

ggsave(filename = '~/R/Alina_RNAseq/GO_fit_plot.tiff', 
       plot = GO_fit, 
       dpi = 300,
       width = 12,
       height = 10,
...
```{r}
GO_fit.dot <- plot(dotplot(GO_results, showCategory = 13))
...
```{r}
GO_fit.cnet <- plot(cnetplot(GO_results, showCategory = 7))

ggsave(filename = '~/R/Alina_RNAseq/GO_cnet_plot.tiff', 
       plot = GO_fit.cnet, 
       dpi = 300,
       width = 12,
       height = 10,
...
```{r}
Downgene_names <- rownames(sigs.df[sigs.df$log2FoldChange< -1,])
...
```{r}
GO_DownReg_results <- enrichGO(gene = Downgene_names,
                       OrgDb = "org.Mm.eg.db",
                       keyType = "ENSEMBL",
                       ont = "BP",
                       pAdjustMethod = "BH",
...
GO.DownReg.df <- as.data.frame(GO_DownReg_results)
GO.DownReg.df
```
