---
title: 'DGE Analysis Notebook - 755 vs HI'
output:
  html_document: 
    toc: yes
    fig_width: 10
...
```{r}
# as DGEList
dge_755b <- DGEList(counts = countsmatrix)
...
```{r}
dds_755b <- DESeqDataSetFromMatrix(countData = countsmatrix,
                              colData = coldata, 
...
saveplot <- function(plot,name ){
  ...
  ggsave(filename = 
           glue('~/R/Alina_RNAseq/755vsHI/{name}.png'),
       plot = plot,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
keep <- rowSums(counts(dds_755b)) > 1
dds_755b <- dds_755b[keep,]
nrow(dds_755b)
```
...
```{r}
dds_755b <- estimateSizeFactors(dds_755b)
dds_755b
```
...
(EuclideanDistanceHeatmap <- pheatmap(sampleDistMatrix,
         ...
         main = "Sample-to-Sample Euclidean Distance of BL6 - 755B",
         col = colors))
...
```{r}
poisd <- PoissonDistance(t(counts(dds_755b))) # raw counts or unnormalised data
samplePoisDistMatrix <- as.matrix( poisd$dd )
rownames(samplePoisDistMatrix) <- dds_755b$Sample_Name
colnames(samplePoisDistMatrix) <- dds_755b$Sample_Name

colors <- colorRampPalette( rev(brewer.pal(9, "RdYlBu")) )(255)
(poisson_dist_plot <- pheatmap(samplePoisDistMatrix,
         clustering_distance_rows = poisd$dd,
         clustering_distance_cols = poisd$dd,
         main = "Sample-to-Sample Poisson Distance of BL6 - 755B",
         col = colors))
...
pca_755b <- prcomp(t(assay(vsd)[select,]))
summary(pca_755b)
# the contribution to the total variance for each component
#percentVar_755b <- (pca_755b$sdev^2 / sum( pca_755b$sdev^2 )) * 100
#percentVar_755b
```
...
savingFunction <- function(plotname, metadatacolumn){
  ...
  ggsave(filename = 
           glue('~/R/Alina_RNAseq/755vsHI/PCAplot_HighVsLowInducer_{metadatacolumn}.png'),
       plot = plotname,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
pcaData_755b <- plotPCA(vsd, intgroup = c("ER_microcolonies",
                                                "Sample_Name"), 
                              returnData = TRUE)
percentVar_755b <- percentvar_calculation(pcaData_755b)
```
...
```{r }
(PCAplot_vst <- ggplot(pcaData_755b,
                      aes(x = PC1,
                          y = PC2,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_755b[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_755b[2], "% variance")) +
        ggtitle("PCA Plot - ER_Microcolonies 755B") +
...
```{r}
PCAdata_755b_MC <- plotPCA(vsd, intgroup = c("Sample_Name", "microcolonies"), returnData = TRUE)
percentVar_755b_MC <- percentvar_calculation(PCAdata_755b_MC)
```

```{r}
(PCAplot_MC <- ggplot(PCAdata_755b_MC,
                     aes(x = PC1, y = PC2,
                         color = microcolonies,
                         label= Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_755b_MC[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_755b_MC[2], "% variance")) +
        ggtitle("PCA Plot - ER_Microcolonies755B - Microcolonies") +
        theme.my.own)
...
```{r}
PCAdata_755b_CO <- plotPCA(vsd, 
                                 intgroup = c("Sample_Name", "clinical_outcome"), 
                                 returnData = TRUE)
percentVar_755b_CO <- percentvar_calculation(PCAdata_755b_CO)
```

```{r}
(PCAplot_CO <- ggplot(PCAdata_755b_CO,
                     aes(x = PC1, y = PC2,
                         color = clinical_outcome,
                         label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_755b_CO[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_755b_CO[2], "% variance")) +
        ggtitle("PCA Plot - ER_Microcolonies755B - Clinical Outcome") +
        theme.my.own)
...
```{r}
PCAdata_755b_PL <- plotPCA(vsd, intgroup = c("Sample_Name", 
                                                   "phylogenomic_lineage"), 
                                 returnData = TRUE)
percentVar_755b_PL <- percentvar_calculation(PCAdata_755b_PL)
```

```{r}
(PCAplot_PL <- ggplot(PCAdata_755b_PL,
                       aes(x = PC1, y = PC2,
                           color = phylogenomic_lineage,
                           label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_755b_PL[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_755b_PL[2], "% variance")) +
        ggtitle("PCA Plot - ER_Microcolonies755B - Phylogenomic Lineage") +
        theme.my.own)
...
```{r}
PCAdata_755b_PG <- plotPCA(vsd, intgroup = c("Sample_Name", "phylogroup"), returnData = TRUE)
percentVar_755b_PG <- percentvar_calculation(PCAdata_755b_PG)
```

```{r}
(PCAplot_PG <- ggplot(PCAdata_755b_PG,
                     aes(x = PC1, y = PC2,
                         color = phylogroup,
                         label= Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_755b_PG[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_755b_PG[2], "% variance")) +
        ggtitle("PCA Plot - ER_Microcolonies755B - Phylogroup") +
        theme.my.own)
...
```{r}
PCAdata_755b_IT <- plotPCA(vsd, intgroup = c("Sample_Name", "Intimin_Type"), returnData = TRUE)
head(PCAdata_755b_IT)
percentVar_755b_IT <- round(100 * attr(PCAdata_755b_IT, "percentVar"), digits = 4)
```

```{r}
(PCAplot_IT <- ggplot(PCAdata_755b_IT,
                     aes(x = PC1, y = PC2,
                         color = Intimin_Type,
                         label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_755b_IT[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_755b_IT[2], "% variance")) +
        ggtitle("PCA Plot - ER_Microcolonies755B - Intimin type") +
        theme.my.own)
...
(Hclust_plot <- pheatmap(rld_cor, 
                        color = heat.colors,
                        main = 'Heirarchical Clustering of Samples - ER_Microcolonies755B - Correlation Matrix'
...
```{r}
res_755b <- results(dds1_755b,contrast = c("ER_microcolonies","HI_LM","HI_HM"))
head(res_755b, 30)
```
...
```{r}
dds2_755b <- DESeq(dds_755b, minReplicatesForReplace = Inf)
res2_755b <- results(dds2_755b, 
                           cooksCutoff = FALSE, 
...
```{r}
res2df_755b <- as.data.frame(res2_755b) # convert the results table to a df
head(res2df_755b)
```
...
```{r}
resultsNames(dds2_755b)
```

```{r}
plotMA_res2_755b <- plotMA(res2_755b, ylim = c(-2, 2))
```

### Histogram of p-values

```{r}
hist(res2_755b$pvalue, breaks = 50, col = "grey50", border = "blue")
```

Further Filtering: baseMean \> 1

```{r}
hist(res2_755b$pvalue[res2_755b$baseMean > 1],
...
```{r}
res2df_755b$symbol <- mapIds(org.Mm.eg.db,
                                   keys = rownames(dds_755b),
                                  column = "SYMBOL",
                                  keytype = "ENSEMBL",
                                   multiVals = "first")
res2df_755b$entrez <- mapIds(org.Mm.eg.db,
                                   keys = rownames(dds_755b),
                                  column = "ENTREZID",
                                  keytype = "ENSEMBL",
                                   multiVals = "first")
...
```{r}
res3df_755b <- res2df_755b %>% filter(!is.na(symbol) & !is.na(entrez))
```
...
```{r}
resOrdered_755b <- res3df_755b[order(res3df_755b$pvalue),]
head(resOrdered_755b)
```

```{r}
write.csv(as.data.frame(resOrdered_755b),
          file = "~/R/Alina_RNAseq/755vsHI/results_DGE_EpithelialResponse.csv")
```
...
```{r}
select <- order(rowMeans(counts(dds2_755b,normalized = TRUE)),
                decreasing = TRUE)[1:20]
df <- as.data.frame(colData(dds2_755b)[,c("ER_microcolonies","Sample_Name")])
```
...
p1 <- EnhancedVolcano(res3df_755b,
    lab = res3df_755b$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
...
ggsave(filename = '~/R/Alina_RNAseq/755vsHI/Volcano1_L2fcVsPvalue.png',
       plot = volcano1,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r fig.height= 10, fig.width= 10}
p2 <- EnhancedVolcano(res3df_755b,
    lab = res3df_755b$symbol,
    x = 'log2FoldChange',
    y = 'padj',
    xlab = bquote(~Log[2]~'FoldChange'),
...
ggsave(filename = '~/R/Alina_RNAseq/755vsHI/Volcano2_L2fcVsPadj.png',
       plot = volcano2,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
res3df_755b$diffexpressed <- "NS"

# if log2Foldchange > 1.0 and pvalue < 0.05, set as "UP"
res3df_755b$diffexpressed[res3df_755b$log2FoldChange > 1.0 & res3df_755b$pvalue < 0.05] <- "UP"

# if log2Foldchange < -1.0 and pvalue < 0.05, set as "DOWN"
res3df_755b$diffexpressed[res3df_755b$log2FoldChange < -1.0 & res3df_755b$pvalue < 0.05] <- "DOWN"

# Create a new column "delabel" to de, that will contain the name of genes differentially expressed (NA in case they are not)
res3df_755b$delabel <- NA
res3df_755b$delabel[res3df_755b$diffexpressed != "NS"] <- res3df_755b$symbol[res3df_755b$diffexpressed != "NS"]
```

Arrive at relevant genes by imposing thresholds.

```{r}
sigsdf_755b <- res3df_755b[(abs(res3df_755b$log2FoldChange) > 1) & (res3df_755b$pvalue < 0.05),]
```
...
```{r}
sigsdf_755b <- filter(sigsdf_755b,
                          symbol != "Xist",
                          symbol != "Jpx", 
                          symbol != "Ftx", 
                          symbol != "Tsx",
...
write.csv(sigsdf_755b ,"~/R/Alina_RNAseq/755vsHI/SignificantGenes_ER_mc755B.csv")
```

### Number of Genes from different strains that are contributing to UP/DOWN regulation.

```{r}
sigsdf_755b_UP <- sigsdf_755b[(sigsdf_755b$log2FoldChange) > 1, ] #UP Regulation Table
sigsdf_755b_DOWN <- sigsdf_755b[(sigsdf_755b$log2FoldChange) < -1, ] #DOWN Regulation Table
```

```{r}
nrow(sigsdf_755b_UP)
nrow(sigsdf_755b_DOWN)
```

### Determine Top20 UP Genes and Top20 DOWN Genes

```{r}
UpGene_755b <- sigsdf_755b_UP[order(-sigsdf_755b_UP$log2FoldChange), ]$symbol
DownGene_755b <- sigsdf_755b_DOWN[order(-sigsdf_755b_DOWN$log2FoldChange), ]$symbol
DE_Genes_table_755b <- as.data.frame(cbind(UpGene_755b,DownGene_755b)) # sorted based on highest Log2FC value
```

```{r}
write.csv(DE_Genes_table_755b ,"~/R/Alina_RNAseq/755vsHI/DE_GenesTable_ER_mc755B.csv")
head(DE_Genes_table_755b, 25)
```
...
sigs2df_755b <- res2df_755b[(abs(res2df_755b$log2FoldChange) > 1) &  (res2df_755b$pvalue < 0.05),]
```
...
```{r}
mat <- counts(dds2_755b, normalized = TRUE)[rownames(sigsdf_755b),]
...
```{r}
mat2 <- counts(dds2_755b, normalized = TRUE)[rownames(sigs2df_755b),]
...
newHP <- Heatmap(mat2.zs, 
        ...
        #width = unit(8, "in"), 
        #height = unit(8, "in")
        row_labels = sigs2df_755b[rownames(mat2.zs),]$symbol
        )
newHP
```
...
LongHeatMap_Allgenes <- Heatmap(mat2.zs, 
        ...
        column_labels = colnames(mat2.zs),
        row_labels = sigs2df_755b[rownames(mat2.zs),]$symbol,
        name = 'Z-Score Heatmap of DE Genes - ER_microcolinies_755B',
        show_row_names = TRUE, 
        use_raster = TRUE, 
        raster_quality = 5,
        )
...
```{r}
sigs1df_755b <- res2df_755b[(res2df_755b$baseMean > 75) & (abs(res2df_755b$log2FoldChange) > 2) & (res2df_755b$pvalue < 0.05),]
mat1 <- counts(dds2_755b, normalized = TRUE)[rownames(sigs1df_755b),]
...
Heatmap(mat1.zs,
        ...
        name = 'DE Genes',
        row_labels = sigs1df_755b[rownames(mat1.zs),]$symbol)
```
...
```{r}
UPgene_ENS_ID <- rownames(sigsdf_755b_UP)

GO_UPRegResults_755b <- enrichGO(gene = UPgene_ENS_ID,
                                       OrgDb = "org.Mm.eg.db",
                                       keyType = "ENSEMBL",
                                       ont = "BP",
                                       pAdjustMethod = "BH",
...
```{r}
GO_UpRegdf_755b <- as.data.frame(GO_UPRegResults_755b)
head(GO_UpRegdf_755b)
```

```{r}
write.csv(GO_UpRegdf_755b ,"~/R/Alina_RNAseq/755vsHI/GO_UpReg_results_ERMC_755B.csv")
```

```{r fig.height=10, fig.width=10}
GO_UPReg_Barplot_755b <- plot(barplot(GO_UPRegResults_755b, showCategory = 22))
saveplot(GO_UPReg_Barplot_755b, "GO_UPReg_Barplot_755b")
```

```{r fig.height=10, fig.width=10}
GO_UPReg_Dotplot_755b <- plot(dotplot(GO_UPRegResults_755b, showCategory = 23))

saveplot(GO_UPReg_Dotplot_755b, "GO_UPReg_Dotplot_755b")
```

```{r fig.height=12, fig.width=10}
GO_UPReg_Cnetplot_755b <- plot(cnetplot(GO_UPRegResults_755b, showCategory = 13))

saveplot(GO_UPReg_Cnetplot_755b, "GO_UPReg_Cnetplot_755b")
```

### Upset Plot

```{r fig.height= 8, fig.width=12}
GO_UPReg_Upsetplot <- plot( upsetplot(GO_UPRegResults_755b))
...
```{r fig.height= 15, fig.width=15}
GO_UPReg_Heatplot <- plot( heatplot(GO_UPRegResults_755b))
...
edox2 <- pairwise_termsim(GO_UPRegResults_755b)
...
```{r}
DOWNgene_ENS_ID <- rownames(sigsdf_755b_DOWN)
```

```{r}
GO_DOWNRegResults_755b <- enrichGO(gene = DOWNgene_ENS_ID,
                       OrgDb = "org.Mm.eg.db",
                       keyType = "ENSEMBL",
                       ont = "BP",
                       pAdjustMethod = "BH",
...
GO_DOWNRegdf_755b <- as.data.frame(GO_DOWNRegResults_755b)
head(GO_DOWNRegdf_755b)
```

```{r}
write.csv(GO_DOWNRegdf_755b,
          "~/R/Alina_RNAseq/755vsHI/GO_DOWNReg_results_ERMC_755B.csv")
```

```{r fig.height= 10, fig.width=10}
GO_DOWNReg_Barplot_755b <- plot(barplot(GO_DOWNRegResults_755b, showCategory = 22))
saveplot(GO_DOWNReg_Barplot_755b, "GO_DOWNReg_Barplot_755b")
```

```{r fig.height=10, fig.width=10}
GO_DOWNReg_Dotplot_755b <- plot(dotplot(GO_DOWNRegResults_755b, showCategory = 23))

saveplot(GO_DOWNReg_Dotplot_755b, "GO_DOWNReg_Dotplot_755b")
```

```{r fig.height= 10, fig.width=10}
GO_DOWNReg_Cnetplot_755b <- plot(cnetplot(GO_DOWNRegResults_755b, 
                                                showCategory = 13
                                                ))

saveplot(GO_DOWNReg_Cnetplot_755b, "GO_DOWNReg_Cnetplot_755b")
```
...
GO_DOWNReg_Upsetplot <- plot( upsetplot(GO_DOWNRegResults_755b))
...
GO_DOWNReg_Heatplot <- plot(heatplot(GO_DOWNRegResults_755b))
...
```{r fig.height= 10, fig.width=15}
edox1 <- pairwise_termsim(GO_DOWNRegResults_755b)
...
entreztable <- unique(dplyr::filter(sigsdf_755b, entrez != "NA"))
```

extract and name the fold changes

```{r}
foldchanges <-  sigsdf_755b$log2FoldChange
names(foldchanges) <-  sigsdf_755b$entrez
...
```{r}
de_l2fc <- as.vector(res3df_755b$log2FoldChange)
# <- res3df_755b$entrez <-
#e_ID <- res3df_755b$entrez
head(de_l2fc)
```
...
```{r}
backgroundgenes <- !is.na(sigsdf_755b$entrez)
...
write.csv(gseaKEGG_results, "~/R/Alina_RNAseq/755vsHI/gseaOE_kegg.csv")
```
...
```{r}
selectUPGenes_entrezid = unique(sigsdf_755b_UP['entrez'] )
selectDOWNGenes_entrezid = unique(sigsdf_755b_DOWN['entrez'] )
```

```{r}
UniverseGenes = unique(sigsdf_755b$entrez)
