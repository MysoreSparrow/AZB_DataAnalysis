---
title: 'DGE Analysis Notebook - ER_Microcolonies: 755 vs HI'
output:
  html_document: 
    toc: yes
    fig_width: 8
...
```{r}
dds_ermc <- DESeqDataSetFromMatrix(countData = countsmatrix,
                              colData = coldata, 
...
saveplot <- function(plot,name ){
  ...
  ggsave(filename = 
           glue('/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/{name}.png'),
       plot = plot,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
keep <- rowSums(counts(dds_ermc)) > 1
dds_ermc <- dds_ermc[keep,]
nrow(dds_ermc)
```
...
```{r}
dds_ermc <- estimateSizeFactors(dds_ermc)
dds_ermc
```
...
(EuclideanDistanceHeatmap <- pheatmap(sampleDistMatrix,
         ...
         main = "Sample-to-Sample Euclidean Distance of BL6 - 755 Versus LI",
         col = colors))
...
```{r}
poisd <- PoissonDistance(t(counts(dds_ermc))) # raw counts or unnormalised data
samplePoisDistMatrix <- as.matrix( poisd$dd )
rownames(samplePoisDistMatrix) <- dds_ermc$Sample_Name
colnames(samplePoisDistMatrix) <- dds_ermc$Sample_Name

colors <- colorRampPalette( rev(brewer.pal(9, "RdYlBu")) )(255)
(poisson_dist_plot <- pheatmap(samplePoisDistMatrix,
         clustering_distance_rows = poisd$dd,
         clustering_distance_cols = poisd$dd,
         main = "Sample-to-Sample Poisson Distance of BL6 - 755 Versus LI",
         col = colors))
...
savingFunction <- function(plotname, metadatacolumn){
  ...
  ggsave(filename = 
           glue('/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/PCAplot_HighVsLowInducer_{metadatacolumn}.png'),
       plot = plotname,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
pcaData_ermc <- plotPCA_local(vsd, intgroup = c("Epithelial_response","Sample_Name"),returnData = T)
pcaData_ermc
...
```
```{r }
(PCAplot_vst <- ggplot(pcaData_ermc,
                      aes(x = PC1,
                          y = PC2,
                          color = Sample_Name,
                          label = Sample_Name)) +
...
(PCAplot_vst23 <- ggplot(pcaData_ermc,
                      aes(x = PC2,
                          y = PC3,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC2: ", percentVar_ermc[2], "% variance")) +
        ylab(paste0("PC3: ", percentVar_ermc[3], "% variance")) +
        ggtitle("PCA Plot - 755vsHI") +
        scale_colour_manual(values = color_values) +
        theme.my.own )

ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/PCAplot23_HighVsLowInducer_ermc.png',
       plot = PCAplot_vst23,
       dpi = 300,
       width = 10,
       height = 10,
...
(PCAplot_vst34 <- ggplot(pcaData_ermc,
                      aes(x = PC3,
                          y = PC4,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC3: ", percentVar_ermc[3], "% variance")) +
        ylab(paste0("PC4: ", percentVar_ermc[4], "% variance")) +
        ggtitle("PCA Plot - 755vsHI") +
        scale_colour_manual(values = color_values) +
        theme.my.own )

ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/PCAplot34_HighVsLowInducer_ermc.png',
       plot = PCAplot_vst34,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
PCAdata_ermc_MC <- plotPCA(vsd, intgroup = c("Sample_Name", "microcolonies"), returnData = TRUE)
percentVar_ermc_MC <- percentvar_calculation(PCAdata_ermc_MC)

(PCAplot_MC <- ggplot(PCAdata_ermc_MC,
                     aes(x = PC1, y = PC2,
                         color = microcolonies,
                         label= Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_ermc_MC[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_ermc_MC[2], "% variance")) +
        ggtitle("PCA Plot - 755vsHI - Microcolonies") +
        theme.my.own)
...
```{r}
PCAdata_ermc_CO <- plotPCA(vsd, 
                                 intgroup = c("Sample_Name", "clinical_outcome"), 
                                 returnData = TRUE)
percentVar_ermc_CO <- percentvar_calculation(PCAdata_ermc_CO)

(PCAplot_CO <- ggplot(PCAdata_ermc_CO,
                     aes(x = PC1, y = PC2,
                         color = clinical_outcome,
                         label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_ermc_CO[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_ermc_CO[2], "% variance")) +
        ggtitle("PCA Plot - 755vsHI - Clinical Outcome") +
        theme.my.own)
...
```{r}
PCAdata_ermc_ERMC <- plotPCA(vsd, intgroup = c("Sample_Name", "ER_microcolonies"), returnData = TRUE)
percentVar_ermc_ERMC <- percentvar_calculation(PCAdata_ermc_ERMC)

(PCAplot_ERMC <- ggplot(PCAdata_ermc_ERMC,
                       aes(x = PC1, y = PC2,
                           color = ER_microcolonies,
                           label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_ermc_ERMC[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_ermc_ERMC[2], "% variance")) +
...
```{r}
PCAdata_ermc_PL <- plotPCA(vsd, intgroup = c("Sample_Name", 
                                                   "phylogenomic_lineage"), 
                                 returnData = TRUE)
percentVar_ermc_PL <- percentvar_calculation(PCAdata_ermc_PL)

(PCAplot_PL <- ggplot(PCAdata_ermc_PL,
                       aes(x = PC1, y = PC2,
                           color = phylogenomic_lineage,
                           label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_ermc_PL[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_ermc_PL[2], "% variance")) +
        ggtitle("PCA Plot - 755vsHI - Phylogenomic Lineage") +
        theme.my.own)
...
```{r}
PCAdata_ermc_PG <- plotPCA(vsd, intgroup = c("Sample_Name", "phylogroup"), returnData = TRUE)
percentVar_ermc_PG <- percentvar_calculation(PCAdata_ermc_PG)

(PCAplot_PG <- ggplot(PCAdata_ermc_PG,
                     aes(x = PC1, y = PC2,
                         color = phylogroup,
                         label= Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_ermc_PG[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_ermc_PG[2], "% variance")) +
        ggtitle("PCA Plot - 755vsHI - Phylogroup") +
        theme.my.own)
...
```{r}
PCAdata_ermc_IT <- plotPCA(vsd, intgroup = c("Sample_Name", "Intimin_Type"), returnData = TRUE)
head(PCAdata_ermc_IT)
percentVar_ermc_IT <- round(100 * attr(PCAdata_ermc_IT, "percentVar"), digits = 4)

(PCAplot_IT <- ggplot(PCAdata_ermc_IT,
                     aes(x = PC1, y = PC2,
                         color = Intimin_Type,
                         label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_ermc_IT[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_ermc_IT[2], "% variance")) +
        ggtitle("PCA Plot - 755vsHI - Intimin type") +
        theme.my.own)
...
(Hclust_plot <- pheatmap(rld_cor, 
                        ...
                        #filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/Hclust_plot.tiff'
                       ))
#Hclust_plot
```
...
```{r}
res_ermc <- results(dds1_ermc, contrast = c("ER_microcolonies","HI_LM","HI_HM"))
head(res_ermc, 30)
```
...
```{r}
dds2_ermc <- DESeq(dds_ermc, minReplicatesForReplace = Inf)
res2_ermc <- results(
  dds2_ermc,
  cooksCutoff = FALSE,
...
```{r}
res2df_ermc <- as.data.frame(res2_ermc) # convert the results table to a df
head(res2df_ermc)
```
...
```{r}
plotMA_res2_ermc <- plotMA(res2_ermc, ylim = c(-2, 2))
```

### Histogram of p-values

```{r}
hist(res2_ermc$pvalue, breaks = 50, col = "grey50", border = "blue")
```

Further Filtering: baseMean \> 1

```{r}
hist(res2_ermc$pvalue[res2_ermc$baseMean > 10],
...
```{r}
res2df_ermc <- tibble::rownames_to_column(res2df_ermc, "symbol")

gn <- res2df_ermc$symbol
...
res2df_ermc$entrez <- entrez
res2df_ermc$ensemblID <- ensembl_id
```

Omit NA values from symbol and respective rows!

```{r}
res3df_ermc <- res2df_ermc %>% filter(!is.na(symbol) & !is.na(entrez))
nrow(res3df_ermc)
```
...
```{r}
resOrdered_ermc <- res3df_ermc[order(res3df_ermc$pvalue),]
head(resOrdered_ermc)

write.csv(as.data.frame(resOrdered_ermc),
          file = "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/results_DGE_ER.csv")
```
...
```{r}
select <- order(rowMeans(counts(dds2_ermc,normalized = FALSE)),decreasing = TRUE)[1:20]
df <- as.data.frame(colData(dds2_ermc)[,c("Epithelial_response","Sample_Name")])
...
```{r fig.height= 10, fig.width= 10}
p1 <- EnhancedVolcano(res3df_ermc,
    lab = res3df_ermc$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
...
ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/Volcano1_L2fcVsPvalue.png',
       plot = volcano1,
       dpi = 300,
       width = 5,
       height = 5,
...
```{r fig.height= 10, fig.width= 10}
p2 <- EnhancedVolcano(res3df_ermc,
    lab = res3df_ermc$symbol,
    x = 'log2FoldChange',
    y = 'padj',
    xlab = bquote(~Log[2]~'FoldChange'),
...
ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/Volcano2_L2fcVsPadj.png',
       plot = volcano2,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
res3df_ermc$diffexpressed <- "NS"

# if log2Foldchange > 1.0 and pvalue < 0.05, set as "UP"
res3df_ermc$diffexpressed[res3df_ermc$log2FoldChange > 1.0 & res3df_ermc$pvalue < 0.05] <- "UP"

# if log2Foldchange < -1.0 and pvalue < 0.05, set as "DOWN"
res3df_ermc$diffexpressed[res3df_ermc$log2FoldChange < -1.0 & res3df_ermc$pvalue < 0.05] <- "DOWN"

# Create a new column "delabel" to de, that will contain the name of genes differentially expressed (NA in case they are not)
res3df_ermc$delabel <- NA
res3df_ermc$delabel[res3df_ermc$diffexpressed != "NS"] <- res3df_ermc$symbol[res3df_ermc$diffexpressed != "NS"]
```

### Arrive at relevant genes by imposing thresholds.

```{r}
sigsdf_ermc <- res3df_ermc[(abs(res3df_ermc$log2FoldChange) > 1) & (res3df_ermc$pvalue < 0.05),]
...
```{r}
sigsdf_ermc <- filter(sigsdf_ermc,
                          symbol != "Xist",
                          symbol != "Jpx", 
                          symbol != "Ftx", 
                          symbol != "Tsx",
...
write.csv(sigsdf_ermc ,"/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/SignificantGenes_ER.csv")
```

### Number of Genes from different strains that are contributing to UP/DOWN regulation.

```{r}
sigsdf_ermc_UP <- sigsdf_ermc[(sigsdf_ermc$log2FoldChange) > 1, ] #UP Regulation Table
sigsdf_ermc_DOWN <- sigsdf_ermc[(sigsdf_ermc$log2FoldChange) < -1, ] #DOWN Regulation Table

nrow(sigsdf_ermc_UP)
nrow(sigsdf_ermc_DOWN)
```

### Determine Top20 UP Genes and Top20 DOWN Genes

```{r}
UpGene_ermc <- sigsdf_ermc_UP[order(-sigsdf_ermc_UP$log2FoldChange,decreasing = TRUE), ]$symbol
DownGene_ermc <- sigsdf_ermc_DOWN[order(-sigsdf_ermc_DOWN$log2FoldChange,decreasing = TRUE), ]$symbol
DE_Genes_table_ermc <- as.data.frame(cbind(UpGene_ermc,DownGene_ermc)) # sorted based on highest Log2FC value
```

```{r}
write.csv(DE_Genes_table_ermc ,"/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/DEGenesTable_ER.csv")
head(DE_Genes_table_ermc, 25)
```
...
sigs2df_ermc <- res2df_ermc[(abs(res2df_ermc$log2FoldChange) > 1) &  (res2df_ermc$pvalue < 0.05),]
```
...
```{r}
#mat <- counts(dds2_ermc, normalized = TRUE)[rownames(sigsdf_ermc),]
mat <- counts(dds2_ermc, normalized = TRUE)[(sigsdf_ermc$symbol),]
...
```{r}
mat2 <- counts(dds2_ermc, normalized = TRUE)[(sigsdf_ermc$symbol),]
...
newHP <- Heatmap(mat2.zs, 
        ...
        #width = unit(8, "in"), 
        #height = unit(8, "in")
        row_labels = sigs2df_ermc[rownames(mat2.zs),]$symbol
        )
newHP
```
...
LongHeatMap_Allgenes <- Heatmap(mat2.zs, 
        ...
        cluster_rows = TRUE, 
        column_labels = colnames(mat2.zs),
        row_labels = sigs2df_ermc$symbol,
        name = 'All DE Genes',
        show_row_names = TRUE, 
        use_raster = TRUE, 
        raster_quality = 5,
...
```{r}
sigs1df_ermc <- res2df_ermc[(res2df_ermc$baseMean > 500) & (abs(res2df_ermc$log2FoldChange) > 1) & (res2df_ermc$pvalue < 0.05),]
mat1 <- counts(dds2_ermc, normalized = TRUE)[(sigs1df_ermc$symbol),]
...
Heatmap(mat1.zs,
        ...
        name = 'DE Genes',
        row_labels = sigs1df_ermc$symbol)
```
...
```{r}
UPgene_ENS_ID <- (sigsdf_ermc_UP$ensemblID)

GO_UPRegResults_ermc <- enrichGO(gene = UPgene_ENS_ID,
                                       OrgDb = "org.Mm.eg.db",
                                       keyType = "ENSEMBL",
                                       ont = "BP",
                                       pAdjustMethod = "BH",
...
```{r}
GO_UpRegdf_ermc <- as.data.frame(GO_UPRegResults_ermc)
head(GO_UpRegdf_ermc)
```

```{r}
write.csv(GO_UpRegdf_ermc ,"/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/GO_UpReg_results_Epithelial.csv")
```

```{r fig.height=10, fig.width=10}
# GO_UPReg_Barplot_ermc <- plot(barplot(GO_UPRegResults_ermc, showCategory = 22))
# saveplot(GO_UPReg_Barplot_ermc, "GO_UPReg_Barplot_ermc")
```

```{r fig.height=10, fig.width=10}
# GO_UPReg_Dotplot_ermc <- plot(dotplot(GO_UPRegResults_ermc, showCategory = 23))
# 
# saveplot(GO_UPReg_Dotplot_ermc, "GO_UPReg_Dotplot_ermc")
```

```{r fig.height=10, fig.width=12}
# GO_UPReg_Cnetplot_ermc <- plot(cnetplot(GO_UPRegResults_ermc, showCategory = 13))
# 
# saveplot(GO_UPReg_Cnetplot_ermc, "GO_UPReg_Cnetplot_ermc")
```

### Upset Plot

```{r fig.height= 8, fig.width=12}
# GO_UPReg_Upsetplot <- plot( upsetplot(GO_UPRegResults_ermc))
...
# GO_UPReg_Heatplot <- plot( heatplot(GO_UPRegResults_ermc))
...
# edox2 <- pairwise_termsim(GO_UPRegResults_ermc)
...
```{r}
DOWNgene_ENS_ID <- (sigsdf_ermc_DOWN$ensemblID)

GO_DOWNRegResults_ermc <- enrichGO(gene = DOWNgene_ENS_ID,
                       OrgDb = "org.Mm.eg.db",
                       keyType = "ENSEMBL",
                       ont = "BP",
                       pAdjustMethod = "BH",
...
GO_DOWNRegdf_ermc <- as.data.frame(GO_DOWNRegResults_ermc)
head(GO_DOWNRegdf_ermc)
```

```{r}
write.csv(GO_DOWNRegdf_ermc,
          "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/GO_DOWNReg_results_Epithelial.csv")
```

```{r}
# GO_DOWNReg_Barplot_ermc <- plot(barplot(GO_DOWNRegResults_ermc))
# saveplot(GO_DOWNReg_Barplot_ermc, "GO_DOWNReg_Barplot_ermc")
```

```{r}
# GO_DOWNReg_Dotplot_ermc <- plot(dotplot(GO_DOWNRegResults_ermc))
# 
# saveplot(GO_DOWNReg_Dotplot_ermc, "GO_DOWNReg_Dotplot_ermc")
```

```{r fig.width=10, fig.height=10}
# GO_DOWNReg_Cnetplot_ermc <- plot(cnetplot(GO_DOWNRegResults_ermc, 
#                                                 showCategory = 13, 
#                                                 circular = FALSE, 
#                                                 colorEdge = FALSE
#                                                 ))
# 
# saveplot(GO_DOWNReg_Cnetplot_ermc, "GO_DOWNReg_Cnetplot_ermc")
```
...
```{r}
# GO_DOWNReg_Upsetplot <- plot( upsetplot(GO_DOWNRegResults_ermc))
...
# GO_DOWNReg_Heatplot <- plot(heatplot(GO_DOWNRegResults_ermc))
...
```{r}
# edox1 <- pairwise_termsim(GO_DOWNRegResults_ermc)
...
# sigsdf_ermc_unique <- unique(dplyr::filter(sigsdf_ermc, entrez != "NA"))
```

extract and name the fold changes

```{r}
# foldchanges <-  sigsdf_ermc$log2FoldChange
# names(foldchanges) <-  sigsdf_ermc$entrez
# 
...
# 
# write.csv(gseaKEGG_results, "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/755vsHI/gseaOE_kegg.csv")
```
...
```{r}
selectUPGenes_entrezid = unique(sigsdf_ermc_UP['entrez'] )
selectDOWNGenes_entrezid = unique(sigsdf_ermc_DOWN['entrez'] )
UniverseGenes = unique(sigsdf_ermc$entrez)
