...
```{r}
res_thresh <- results(dds1, alpha = 0.05, lfcThreshold=1)
res_thresh
```
...
'anno' is a data.frame that instructs the pheatmap function how to label the columns.
```{r}
anno <- as.data.frame(colData(rld)[, c("condition","Samples")])
str(anno)
```
...
df <- as.data.frame(colData(dds1)[,c("condition","Samples")])
```
...
```{r}
write.csv(as.data.frame(resOrdered), file = "C:/Users/User/Documents/bam/results.csv")
```
