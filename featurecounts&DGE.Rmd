...
coldata$Samples = str_remove_all(coldata$Samples, pattern = '_.bam') # tidying up the names
coldata$Samples = str_remove_all(coldata$Samples, pattern = 'run6_trimmed_') # tidying up the names
```
...
```{r}
PCAplot_vst <- ggplot(pcaData, aes(x = PC1, y = PC2, color = Samples, label= Samples)) +
  geom_point(size =2) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  coord_fixed() +
  ggtitle("PCA with VST data")+ 
  theme(legend.position="bottom") + theme(aspect.ratio=10/36) +
  geom_text(size =2, hjust=0, vjust=0)

ggsave(filename = 'C:/Users/User/Documents/bam/PCA_vst_plot1.tiff', plot = PCAplot_vst)
```

```{r}
ggplot(pcaData, aes(x = PC1, y = PC2, color = Samples, label= Samples)) +
  geom_point(size =2) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  coord_fixed() +
...
```{r}
ggplot(gpca.dat, aes(x = dim1, y = dim2, color = Samples, shape = condition, label = Samples)) +
...
### Applying pvalue and LFC thresholds!
```{r}
#res2.df$log2FoldChange <- (abs(res2.df$log2FoldChange>1)) 
#res2.df <- res2.df [(res2.df$pvalue>0.05)]
```
...
```{r}
write.csv(as.data.frame(resOrdered), file = "C:/Users/User/Documents/bam/results2.csv")
```
...
```{r}
  p1 +
  ggplot2::coord_cartesian(ylim=c(-2, 12)) +
  ...
  coord_fixed(ratio = 15/55)
  
```

TODO : make the plot look aesthetically better and also show gene symbols instead of geneIDs.
...
```
```{r}
mat <- counts(dds2, normalized = TRUE)[rownames(res2.df),]
head(mat)
```

Calculating the zscore for each row

```{r}
mat.zs <- t(apply(mat, 1, scale))
...
Heatmap(mat.zs,
        ...
        row_labels = res2.df[rownames(mat.zs),]$symbol)
```
