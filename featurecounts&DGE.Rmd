...
PCAplot_vst <- ggplot(pcaData, aes(x = PC1, y = PC2, color = Samples, label= Samples)) +
  ...
  theme(legend.position="bottom") + theme(aspect.ratio=10/26) #+geom_text(hjust=0, vjust=0)
...
ggplot(pcaData, aes(x = PC1, y = PC2, color = Samples, label= Samples)) +
  ...
  coord_fixed() +
  ggtitle("PCA with VST data")+ 
  theme(legend.position="bottom") + theme(aspect.ratio=10/26) +
...
topVarGenes <- order(rowVars(assay(vsd)), decreasing = TRUE)
head(topVarGenes, 20)
```
...
```{r}
anno <- as.data.frame(colData(vsd)[, c("condition","Samples")])
```
