---
title: 'DGE Analysis Notebook: BL6 Infected Vs Control'
output:
  html_document: 
    toc: yes
    fig_width: 6
...
savingFunction <- function(plotname, metadatacolumn){
  ...
  ggsave(filename = 
           glue('/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/PCAplot_InfectedvsControl_{metadatacolumn}.png'),
       plot = plotname,
       dpi = 300,
       width = 10,
       height = 10,
...
(PCAplot_vst <- ggplot(pcaData_infected,
        ...
        ggtitle("PCA Plot - InfectedvsControl") +
...
(PCAplot_vst23 <- ggplot(pcaData_infected,
                      aes(x = PC2,
                          y = PC3,
                          color = Sample_Name,
                          label = Sample_Name)) +
...
ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/PCAplot23_InfectedvsControl.png',
       plot = PCAplot_vst23,
       dpi = 300,
       width = 10,
       height = 10,
...
(PCAplot_vst34 <- ggplot(pcaData_infected,
                      aes(x = PC3,
                          y = PC4,
                          color = Sample_Name,
                          label = Sample_Name)) +
...
ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/PCAplot34_InfectedvsControl.png',
       plot = PCAplot_vst34,
       dpi = 300,
       width = 10,
       height = 10,
...
(PCAplot_ER <- ggplot(PCAdata_infected_ER,
        ...
        ggtitle("PCA Plot - InfectedvsControl - Epithelial_response") +
        theme.my.own)
...
```{r}
PCAdata_infected_MC <- plotPCA(vsd, intgroup = c("Sample_Name", "microcolonies"), 
                               returnData = TRUE)
...
(PCAplot_MC <- ggplot(PCAdata_infected_MC,
        ...
        ggtitle("PCA Plot - InfectedvsControl - Microcolonies") +
        theme.my.own)
...
(PCAplot_CO <- ggplot(PCAdata_infected_CO,
        ...
        ggtitle("PCA Plot - InfectedVs Control Inducer - Clinical Outcome") +
        theme.my.own)
...
(PCAplot_ERMC <- ggplot(PCAdata_infected_ERMC,
        ...
        ggtitle("PCA Plot - InfectedVs Control Inducer - ER_Microcolonies") +
        theme.my.own)
...
(PCAplot_PL <- ggplot(PCAdata_infected_PL,
        ...
        ggtitle("PCA Plot - InfectedVs Control Inducer - Phylogenomic Lineage") +
        theme.my.own)
...
(PCAplot_PG <- ggplot(PCAdata_infected_PG,
        ...
        ggtitle("PCA Plot - InfectedVs Control Inducer - Phylogroup") +
        theme.my.own)
...
(PCAplot_IT <- ggplot(PCAdata_infected_IT,
        ...
        ggtitle("PCA Plot - InfectedVs Control Inducer - Intimin type") +
        theme.my.own)
...
write.csv(as.data.frame(resOrdered_infected),
          file = "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/results_DGE.csv")
```
...
ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/Volcano1_L2fcVsPvalue.png',
       plot = volcano1,
       dpi = 300,
       width = 5,
       height = 5,
...
(volcano2 <- p2 + scale_y_continuous(limits = c(0, 4),
                                    breaks = seq(0, 4, 1),
                                    sec.axis = sec_axis(~. *1,
                                                        labels = NULL,
                                                        breaks = NULL)) +
...
```
```{r}
write.csv(UpGene_infected,
          "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/UpGenelist_infectedvscontrol.csv")
write.csv(DownGene_infected,
          "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/DownGenelist_infected_infectedvscontrol.csv")
```
...
```{r}
sigs1df_infected <- res2df_infected[(res2df_infected$baseMean > 500) & (abs(res2df_infected$log2FoldChange) > 1) & (res2df_infected$pvalue < 0.05),]
...
```{r}
write.csv(GO_UpRegdf_infected ,"/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/GO_UpReg_results_Epithelial.csv")
```
...
#           "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/GO_DOWNReg_results_Epithelial.csv")
```
...
# 
# write.csv(gseaKEGG_results, "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/gseaOE_kegg.csv")
```
...
```{r}
write.csv(l2fctable, "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/IvC/l2fctable_InfectedvsControl_BL6.csv")
```
...
```{r}
#2. Results
res_gt <- res2df_infected
res_gt <- res_gt[!duplicated(res_gt[c('ensemblID')]), ]
res_gt <- as.data.frame(res_gt)

rownames(res_gt) <- res_gt[,9]
#res_gt <- subset(res_gt, select = - ensemblID)
```

```{r}
#3. res_enrich
res_enrich_infected <- shake_enrichResult(GO_UPRegResults_infected)
```

```{r}
colnames(res_enrich_infected)
```
```{r}
res_enrich_infected
```
...
library(pcaExplorer)
#4. annoattion data frame

#anno_df <- data.frame(res_gt$ensemblID, res_gt$symbol)
anno_df <- pcaExplorer::get_annotation_orgdb(dds1_infected, "org.Mm.eg.db", "SYMBOL")
```

## Getting Aggregate Scores

```{r}
# res_enrich_infected <- get_aggrscores(res_enrich = res_enrich_infected,
#                                       res_de = res_gt,
...
```{r}
GeneTonic(dds = dds1_infected,
          res_de = res_infected,
          res_enrich = res_enrich_infected,
          annotation_obj = anno_df,
          project_id = "Alina_1"
          )
```
