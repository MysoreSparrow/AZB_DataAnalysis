...
Hclust_plot <- pheatmap(rld_cor, 
                        ...
                        filename = '~/R/Rtuts/Data/Alina_EPEC_project/plots/Hclust_plot.tiff'
                       )
...
```
```{r}
corrplot(rld_cor, type="upper", order="hclust") # colorful number
```

#### Point of Discussion on Variance between R1 and R2: 

From Poisson plot, Euclidean distance plot, PCA and Heirarchical Clustering based on correlation matrix, 
can we say that R1 is reasonably close enough to R2. So we either drop R2 or we take a mean between R1 and R2.
...
```{r}
res3.df <- res2.df %>% filter(!is.na(symbol)) 
nrow(res3.df)
```
...
res3.df$delabel <- NA
res3.df$delabel[res3.df$diffexpressed != "NO"] <- res3.df$symbol[res3.df$diffexpressed != "NO"]
```
...
p <- ggplot(res3.df, aes(x=log2FoldChange , y = -log10(pvalue),
  ...
  scale_y_continuous(limits = c(-1, 10) ,breaks=seq(-1, 10, 1.0)) +
  scale_x_continuous(limits = c(-4, 9) , breaks=seq(-4, 9, 1.0)) +
...
```{r}
ggplot(res3.df, 
       aes(x=log2FoldChange , y = log10(baseMean))
       ) + 
  geom_point()
...
ggplot(sigs1.df, 
  ...
  geom_point()+
  ggtitle("Log2FC vs -Log10(pvalue)")
  geom_text(aes(label=symbol))
```
...
```
```{r}
ggsave(filename = '~/R/Rtuts/Data/Alina_EPEC_project/plots/GO_fit_dotplot.tiff', 
       plot = GO_fit.dot, 
       dpi = 300,
       width = 12,
       height = 10,
