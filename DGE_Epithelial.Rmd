...
```{r}
pcaData_epithelial <- plotPCA_local(vsd, intgroup = c("Epithelial_response","Sample_Name"),returnData = T)
pcaData_epithelial
...
(PCAplot_vst23 <- ggplot(pcaData_epithelial,
                      aes(x = PC2,
                          y = PC3,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC2: ", percentVar_epithelial[2], "% variance")) +
        ylab(paste0("PC3: ", percentVar_epithelial[3], "% variance")) +
        ggtitle("PCA Plot - High Vs Low Inducer") +
        scale_colour_manual(values = color_values) +
        theme.my.own )

ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/ER/PCAplot23_HighVsLowInducer_epithelial.png',
       plot = PCAplot_vst23,
       dpi = 300,
       width = 10,
       height = 10,
...
(PCAplot_vst34 <- ggplot(pcaData_epithelial,
                      aes(x = PC3,
                          y = PC4,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC3: ", percentVar_epithelial[3], "% variance")) +
        ylab(paste0("PC4: ", percentVar_epithelial[4], "% variance")) +
        ggtitle("PCA Plot - High Vs Low Inducer") +
        scale_colour_manual(values = color_values) +
        theme.my.own )

ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/ER/PCAplot34_HighVsLowInducer_epithelial.png',
       plot = PCAplot_vst34,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
hist(res2_epithelial$pvalue[res2_epithelial$baseMean > 10],
...
```{r}
res2df_epithelial <- tibble::rownames_to_column(res2df_epithelial, "symbol")

gn <- res2df_epithelial$symbol
...
res2df_epithelial$entrez <- entrez
res2df_epithelial$ensemblID <- ensembl_id
```
...
```{r}
select <- order(rowMeans(counts(dds2_epithelial,normalized = FALSE)),decreasing = TRUE)[1:20]
...
```{r}
UpGene_epithelial <- sigsdf_epithelial_UP[order(-sigsdf_epithelial_UP$log2FoldChange,decreasing = TRUE), ]$symbol
DownGene_epithelial <- sigsdf_epithelial_DOWN[order(-sigsdf_epithelial_DOWN$log2FoldChange,decreasing = TRUE), ]$symbol
...
mat <- counts(dds2_epithelial, normalized = TRUE)[(sigsdf_epithelial$symbol),]
...
```{r}
mat2 <- counts(dds2_epithelial, normalized = TRUE)[(sigsdf_epithelial$symbol),]
...
LongHeatMap_Allgenes <- Heatmap(mat2.zs, 
        ...
        cluster_rows = TRUE, 
        column_labels = colnames(mat2.zs),
        row_labels = sigs2df_epithelial$symbol,
        name = 'All DE Genes',
        show_row_names = TRUE, 
        use_raster = TRUE, 
        raster_quality = 5,
...
```{r}
sigs1df_epithelial <- res2df_epithelial[(res2df_epithelial$baseMean > 500) & (abs(res2df_epithelial$log2FoldChange) > 1) & (res2df_epithelial$pvalue < 0.05),]
mat1 <- counts(dds2_epithelial, normalized = TRUE)[(sigs1df_epithelial$symbol),]
...
Heatmap(mat1.zs,
        ...
        name = 'DE Genes',
        row_labels = sigs1df_epithelial$symbol)
```
...
```{r}
UPgene_ENS_ID <- (sigsdf_epithelial_UP$ensemblID)
...
```{r}
DOWNgene_ENS_ID <- (sigsdf_epithelial_DOWN$ensemblID)
