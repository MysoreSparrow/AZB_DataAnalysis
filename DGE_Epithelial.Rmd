...
```{r}
# But isnt the mus musculus code is mmu ?
data("go.sets.mm")
data("go.subs.mm")
```
...
```
Now, process the results to pull out the top 20 upregulated pathways, then further process that just to get the IDs. I can use these KEGG pathway IDs downstream for plotting.
...
The pathview() function in the pathview package makes the plots. The function is written below to loop through and draw plots for the top 20 pathways we created above.
