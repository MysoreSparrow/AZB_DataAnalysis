...
saveplot <- function(plot,name ){
  ...
  ggsave(filename = 
           glue('~/R/Alina_RNAseq/EpithelialResponse/{name}.png'),
       plot = plot,
       dpi = 300,
       width = 10,
       height = 10,
...
theme.my.own <- list(theme_bw() ,
                      geom_point(size = 3),
                      coord_fixed() ,
                      scale_y_continuous(breaks = seq(-20, 20, 5), sec.axis = sec_axis(~. *1)) ,
...
clusterProfiler requires the genes to be identified using Entrez IDs for all genes in our results dataset. We also need to remove the NA values and duplicates (due to gene ID conversion) prior to the analysis
```{r}
## Remove any NA values (reduces the data by quite a bit) and Remove any Entrez duplicates
entrez <- unique(dplyr::filter(sigsdf_epithelial, entrez != "NA"))
```
...
![mmu04144](pathwayimages/mmu04510.pathview.png)
![mmu04144](pathwayimages/mmu04512.pathview.png)
![mmu04144](pathwayimages/mmu04270.pathview.png)

![mmu04144](pathwayimages/mmu04810.pathview.png)

![mmu04144](pathwayimages/mmu04610.pathview.png)

![mmu04144](pathwayimages/mmu04020.pathview.png)
![mmu04144](pathwayimages/mmu04640.pathview.png)

![mmu04144](pathwayimages/mmu04514.pathview.png)
...
write.csv(gseaKEGG_results, "~/R/Alina_RNAseq/EpithelialResponse/gseaOE_kegg.csv")
```
