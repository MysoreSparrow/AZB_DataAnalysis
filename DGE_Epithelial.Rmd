...
```{r}
# the contribution to the total variance for each component
percentVar_epithelial1 <- (pca_epithelial$sdev^2 / sum( pca_epithelial$sdev^2 )) * 100
summary(pca_epithelial)
```
...
var_explained <- pca_epithelial$sdev^2/sum(pca_epithelial$sdev^2)
var_explained
```

trialcode 2
```{r}
(PCAplot_vst <- ggplot(pca_df,
                      aes(x = PC3,
                          y = PC4)) +
...
trial code1
```{r}
pca_df <- as.data.frame(pca_epithelial$x )
pca_df <- rownames_to_column(pca_df)
# Change colname of one column
colnames(pca_df)[colnames(pca_df) == "rowname"] <- "samplename"

  ggplot(pca_df, aes(x = PC3,y = PC4, label = samplename)) + 
  geom_point( size = 2) +
  geom_point(aes(color = samplename),size = 3) +
  coord_fixed() + 
  theme_bw() + 
  labs(x = paste0("PC3: ",round(var_explained[3]*100,3),"%"),
       y = paste0("PC4: ",round(var_explained[4]*100,3),"%")
       ) #+scale_colour_manual(values = color_values) 
```
