...
pcaData_epithelial
percentVar_epithelial <- percentvar_calculation(pcaData_epithelial1)
```
...
```{r}
# genes_epithelial <- mapIds(org.Mm.eg.db, 
#                                keys = rownames(res.pca$var$contrib),
#                                column = "SYMBOL",
#                                keytype ="ENSEMBL")
...
# annotation_epithelial <- annotation_epithelial[rownames(res.pca$var$contrib),]
# rownames(res.pca$var$contrib)=annotation_epithelial
# 
# rownames(res.pca$var$contrib)[!is.na(rownames(res.pca$var$contrib))]
# 
# res.pca$var$contrib <- res.pca$var$contrib[!is.na(rownames(res.pca$var$contrib)),]
# res.pca$var$cor <- res.pca$var$cor[!is.na(rownames(res.pca$var$contrib)),]
# res.pca$var$cos2 <- res.pca$var$cos2 [!is.na(rownames(res.pca$var$contrib)),]
# res.pca$var$coord <- res.pca$var$coord[!is.na(rownames(res.pca$var$contrib)),]
# # rownames(res.pca$var$cor)[!is.na(rownames(res.pca$var$contrib))]
# # rownames(res.pca$var$cos2)[!is.na(rownames(res.pca$var$contrib))]
# # rownames(res.pca$var$coord)[!is.na(rownames(res.pca$var$contrib))]
```
```{r}
...
```
```{r}
# dds_epithelial <- DESeqDataSetFromMatrix(
#   countData = countsmatrix,
#   colData = coldata,
#   design = ~ Epithelial_response
# )
# dds <- DESeq(dds_epithelial)
# vst <- assay(vst(dds_epithelial, blind = FALSE))
```

```{r fig.width=10, fig.height=10}
# p1 <- prcomp(vst, metadata = coldata)
# 
# biplot(p1, showLoadings = TRUE,labSize = 5, pointSize = 5, sizeLoadingsNames = 3)
```
...
genenames_epithelial <- mapIds(org.Mm.eg.db, 
