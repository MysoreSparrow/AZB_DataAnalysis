...
```
Calculating the variance for each PCA value
```{r}
...
(PCAplot_vst <- ggplot(pcaData_epithelial,
        ...
        theme.my.own )
ggsave(filename = '~/R/Alina_RNAseq/EpithelialResponse/PCAplot_HighVsLowInducer_EpithelialResponse.png',
       plot = PCAplot_vst,
       dpi = 300,
       width = 10,
       height = 10,
...
percentVar_epithelial_MC <- round(100 * attr(PCAdata_epithelial_MC, "percentVar"), digits = 4)
```
...
ggsave(filename = '~/R/Alina_RNAseq/EpithelialResponse/PCAplot_HighVsLowInducer_Microcolonies.png',
       plot = PCAplot_MC,
       dpi = 300,
       width = 10,
       height = 10,
...
percentVar_epithelial_CO <- round(100 * attr(PCAdata_epithelial_CO, "percentVar"), digits = 4)
```
...
ggsave(filename = '~/R/Alina_RNAseq/EpithelialResponse/PCAplot_HighVsLowInducer_ClinicalOutcome.png',
       plot = PCAplot_CO,
       dpi = 300,
       width = 10,
       height = 10,
...
percentVar_epithelial_ERMC <- round(100 * attr(PCAdata_epithelial_ERMC, "percentVar"), digits = 4)
```
...
percentVar_epithelial_PL <- round(100 * attr(PCAdata_epithelial_PL, "percentVar"), digits = 4)
```
...
ggsave(filename = '~/R/Alina_RNAseq/EpithelialResponse/PCAplot_HighVsLowInducer_PhylogenomicLineage.png',
       plot = PCAplot_PL,
       dpi = 300,
       width = 10,
       height = 10,
...
percentVar_epithelial_PG <- round(100 * attr(PCAdata_epithelial_PG, "percentVar"), digits = 4)
```
...
ggsave(filename = '~/R/Alina_RNAseq/EpithelialResponse/PCAplot_HighVsLowInducer_Phylogroup.png',
       plot = PCAplot_PG,
       dpi = 300,
       width = 10,
       height = 10,
...
ggsave(filename = '~/R/Alina_RNAseq/EpithelialResponse/PCAplot_HighVsLowInducer_IntiminType.png',
       plot = PCAplot_IT,
       dpi = 300,
       width = 10,
       height = 10,
...
cluster profiler provides gseGO() for gene set enrichment analysis using gene ontology.

```{r}
# Create a sorted/ordered gene list
head(sigsdf_epithelial)
## feature 1: numeric vector
## assume 1st column is ID
## 2nd column is log2FC values
genelist <- sigsdf_epithelial$log2FoldChange
## feature 2: named vector
names(geneList) <- as.character(d[, 1])
```
```{r}
gseGO_UP <- gseGO(geneList = order(sigsdf_epithelial$log2FoldChange,
                                   decreasing = TRUE),
                  OrgDb = org.Mm.eg.db,
                  ont = "BP",
                  minGSSize = 100,
...
```
```{r}
foldchanges <- sigsdf_epithelial_DOWN$log2FoldChange
#names(foldchanges) = sigsdf_epithelial_DOWN$entrez
#head(foldchanges)
foldchanges <- sigsdf_epithelial_DOWN$entrez
head(foldchanges)
```
