---
title: 'DGE Analysis Notebook - Microcolonies: HighVsLow'
output:
  html_document: 
    toc: yes
    fig_width: 10
...
saveplot <- function(plot,name ){
  ...
  ggsave(filename = 
           glue('/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/{name}.png'),
       plot = plot,
       dpi = 300,
       width = 10,
       height = 10,
...
savingFunction <- function(plotname, metadatacolumn){
  ...
  ggsave(filename = 
           glue('/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/PCAplot_HighVsLowInducer_{metadatacolumn}.png'),
       plot = plotname,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
PCAdata_microcolonies_CO <- plotPCA(vsd, 
...
```{r}
PCAdata_microcolonies_erer <- plotPCA(vsd, intgroup = c("Sample_Name", "er_microcolonies"), returnData = TRUE)
percentVar_microcolonies_erer <- percentvar_calculation(PCAdata_microcolonies_erer)

(PCAplot_erer <- ggplot(PCAdata_microcolonies_erer,
                       aes(x = PC1, y = PC2,
                           color = er_microcolonies,
                           label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_microcolonies_erer[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_microcolonies_erer[2], "% variance")) +
        ggtitle("PCA Plot - High Vs Low Inducer - er_Microcolonies") +
        theme.my.own)

savingFunction(PCAplot_erer, "er_microcolonies")
```
...
```{r}
res_microcolonies <- results(dds1_microcolonies,contrast = c("microcolonies","High","Low"))
...
res2_microcolonies <- results(dds2_microcolonies, 
                           cooksCutoff = FALSE, 
...
```{r}
plotMA_res2_microcolonies <- plotMA(res2_microcolonies)
```

```{r}
#res3_microcolonies <- lfcShrink(dds2_microcolonies,coef = "Microcolonies_HighInducer_vs_LowInducer",
#type = "apeglm")
...
write.csv(as.data.frame(resOrdered_microcolonies),
          file = "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/results_DGE_MC.csv")
```
...
ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/Volcano1_L2fcVsPvalue.png',
       plot = volcano1,
       dpi = 300,
       width = 10,
       height = 10,
...
ggsave(filename = '/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/Volcano2_L2fcVsPadj.png',
       plot = volcano2,
       dpi = 300,
       width = 10,
       height = 10,
...
write.csv(sigsdf_microcolonies ,"/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/SignificantGenes_er.csv")
```
...
```{r}
write.csv(DE_Genes_table_microcolonies ,"/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/DEGenesTable_er.csv")
head(DE_Genes_table_microcolonies, 25)
```
...
head(GO_UpRegdf_microcolonies)
```

```{r}
write.csv(GO_UpRegdf_microcolonies ,"/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/GO_UpReg_results_Epithelial.csv")
```

```{r fig.height=10, fig.width=10}
GO_UPReg_Barplot_microcolonies <- plot(barplot(GO_UPRegResults_microcolonies, showCategory = 22))
saveplot(GO_UPReg_Barplot_microcolonies, "GO_UPReg_Barplot_microcolonies")
```

```{r fig.height=10, fig.width=10}
GO_UPReg_Dotplot_microcolonies <- plot(dotplot(GO_UPRegResults_microcolonies, showCategory = 23))

saveplot(GO_UPReg_Dotplot_microcolonies, "GO_UPReg_Dotplot_microcolonies")
```

```{r fig.height=12, fig.width=12}
GO_UPReg_Cnetplot_microcolonies <- plot(cnetplot(GO_UPRegResults_microcolonies, showCategory = 13))

saveplot(GO_UPReg_Cnetplot_microcolonies, "GO_UPReg_Cnetplot_microcolonies")
```

### Upset Plot

```{r fig.height= 8, fig.width=12}
GO_UPReg_Upsetplot <- plot( upsetplot(GO_UPRegResults_microcolonies))
...
GO_UPReg_Heatplot <- plot( heatplot(GO_UPRegResults_microcolonies))
...
edox2 <- pairwise_termsim(GO_UPRegResults_microcolonies)
...
head(GO_DOWNRegdf_microcolonies)
```

```{r}
write.csv(GO_DOWNRegdf_microcolonies,
          "/home/keshavprasadgubbi/Documents/AlinaRnaSeq/MC/GO_DOWNReg_results_Epithelial.csv")
```

```{r}
# GO_DOWNReg_Barplot_microcolonies <- plot(barplot(GO_DOWNRegResults_microcolonies))
# saveplot(GO_DOWNReg_Barplot_microcolonies, "GO_DOWNReg_Barplot_microcolonies")
```

```{r fig.height=10, fig.width=10}
# GO_DOWNReg_Dotplot_microcolonies <- plot(dotplot(GO_DOWNRegResults_microcolonies))
# 
# saveplot(GO_DOWNReg_Dotplot_microcolonies, "GO_DOWNReg_Dotplot_microcolonies")
```

```{r }
# GO_DOWNReg_Cnetplot_microcolonies <- plot(cnetplot(GO_DOWNRegResults_microcolonies, 
#                                                 showCategory = 13, 
#                                                 circular = FALSE, 
#                                                 colorEdge = TRUE
#                                                 ))
# 
# saveplot(GO_DOWNReg_Cnetplot_microcolonies, "GO_DOWNReg_Cnetplot_microcolonies")
```
...
```{r}
# GO_DOWNReg_Upsetplot <- plot( upsetplot(GO_DOWNRegResults_microcolonies))
...
```{r}
# GO_DOWNReg_Heatplot <- plot(heatplot(GO_DOWNRegResults_microcolonies))
...
```{r}
# edox1 <- pairwise_termsim(GO_DOWNRegResults_microcolonies)
...
sigsdf_microcolonies_unique <- unique(dplyr::filter(sigsdf_microcolonies, entrez != "NA"))
```
