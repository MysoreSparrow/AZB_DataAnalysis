...
```{r}
dds_Hposition <- DESeqDataSetFromMatrix(countData = countsmatrix,
                              colData = coldata, 
                              design = ~ HPosition)
nrow(dds_Hposition)
```
...
saveplot <- function(plot,name ){
  ...
  ggsave(filename = 
           glue('~/R/Alina_RNAseq/Hposition/{name}.png'),
       plot = plot,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
keep <- rowSums(counts(dds_Hposition)) > 1
dds_Hposition <- dds_Hposition[keep,]
nrow(dds_Hposition)
```
...
```{r}
vsd <- vst(dds_Hposition, blind = FALSE)
#head(assay(vsd), 3)
colData(vsd)
vsd_coldata <- colData(vsd)
dds_Hposition <- estimateSizeFactors(dds_Hposition)
dds_Hposition
```
...
(EuclideanDistanceHeatmap <- pheatmap(sampleDistMatrix,
         ...
         main = "Sample-to-Sample Euclidean Distance of BL6 - Left vs Right",
         col = colors))
```
...
```{r}
poisd <- PoissonDistance(t(counts(dds_Hposition))) # raw counts or unnormalised data
samplePoisDistMatrix <- as.matrix( poisd$dd )
rownames(samplePoisDistMatrix) <- dds_Hposition$Sample_Name
colnames(samplePoisDistMatrix) <- dds_Hposition$Sample_Name
colors <- colorRampPalette( rev(brewer.pal(9, "RdYlBu")) )(255)
(poisson_dist_plot <- pheatmap(samplePoisDistMatrix,
         clustering_distance_rows = poisd$dd,
         clustering_distance_cols = poisd$dd,
         main = "Sample-to-Sample Poisson Distance of BL6 - Left vs Right",
         col = colors))
...
savingFunction <- function(plotname, metadatacolumn){
  ...
  ggsave(filename = 
           glue('~/R/Alina_RNAseq/Hposition/PCAplot_Hposition_{metadatacolumn}.png'),
       plot = plotname,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
pcaData_Hposition <- plotPCA_local(vsd, intgroup = c("condition","Sample_Name"),returnData = T)
pcaData_Hposition
percentVar_Hposition <- percentvar_calculation(pcaData_Hposition)
```

```{r }
percentVar_Hposition
```
```{r }
(PCAplot_vst <- ggplot(pcaData_Hposition,
                      aes(x = PC1,
                          y = PC2,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right") +
...
```{r}
(PCAplot_vst23 <- ggplot(pcaData_Hposition,
                      aes(x = PC2,
                          y = PC3,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC2: ", percentVar_Hposition[2], "% variance")) +
        ylab(paste0("PC3: ", percentVar_Hposition[3], "% variance")) +
        ggtitle("PCA Plot - Left vs Right") +
        scale_colour_manual(values = color_values) +
        theme.my.own )
ggsave(filename = '~/R/Alina_RNAseq/Hposition/PCAplot23_HpositionvsControl.png',
       plot = PCAplot_vst23,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
(PCAplot_vst34 <- ggplot(pcaData_Hposition,
                      aes(x = PC3,
                          y = PC4,
                          color = Sample_Name,
                          label = Sample_Name)) +
        xlab(paste0("PC3: ", percentVar_Hposition[3], "% variance")) +
        ylab(paste0("PC4: ", percentVar_Hposition[4], "% variance")) +
        ggtitle("PCA Plot - Left vs Right") +
        scale_colour_manual(values = color_values) +
        theme.my.own )
ggsave(filename = '~/R/Alina_RNAseq/Hposition/PCAplot34_HpositionvsControl.png',
       plot = PCAplot_vst34,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
PCAdata_Hposition_ER <- plotPCA(vsd, intgroup = c("Sample_Name", "Epithelial_response"), returnData = TRUE)
percentVar_Hposition_ER <- percentvar_calculation(PCAdata_Hposition_ER)
(PCAplot_ER <- ggplot(PCAdata_Hposition_ER,
                     aes(x = PC1, y = PC2,
                         color = Epithelial_response,
                         label= Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition_ER[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition_ER[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right - Epithelial_response") +
        theme.my.own)
...
```{r}
PCAdata_Hposition_MC <- plotPCA(vsd, intgroup = c("Sample_Name", "microcolonies"), returnData = TRUE)
percentVar_Hposition_MC <- percentvar_calculation(PCAdata_Hposition_MC)
(PCAplot_MC <- ggplot(PCAdata_Hposition_MC,
                     aes(x = PC1, y = PC2,
                         color = microcolonies,
                         label= Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition_MC[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition_MC[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right - Microcolonies") +
        theme.my.own)
...
```{r}
PCAdata_Hposition_CO <- plotPCA(vsd, 
                                 intgroup = c("Sample_Name", "clinical_outcome"), 
                                 returnData = TRUE)
percentVar_Hposition_CO <- percentvar_calculation(PCAdata_Hposition_CO)
(PCAplot_CO <- ggplot(PCAdata_Hposition_CO,
                     aes(x = PC1, y = PC2,
                         color = clinical_outcome,
                         label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition_CO[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition_CO[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right - Clinical Outcome") +
        theme.my.own)
...
```{r}
PCAdata_Hposition_ERMC <- plotPCA(vsd, intgroup = c("Sample_Name", "ER_microcolonies"), 
                                  returnData = TRUE)
percentVar_Hposition_ERMC <- percentvar_calculation(PCAdata_Hposition_ERMC)
(PCAplot_ERMC <- ggplot(PCAdata_Hposition_ERMC,
                       aes(x = PC1, y = PC2,
                           color = ER_microcolonies,
                           label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition_ERMC[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition_ERMC[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right - ER_Microcolonies") +
        theme.my.own)
...
```{r}
PCAdata_Hposition_PL <- plotPCA(vsd, intgroup = c("Sample_Name", 
                                                   "phylogenomic_lineage"), 
                                 returnData = TRUE)
percentVar_Hposition_PL <- percentvar_calculation(PCAdata_Hposition_PL)
(PCAplot_PL <- ggplot(PCAdata_Hposition_PL,
                       aes(x = PC1, y = PC2,
                           color = phylogenomic_lineage,
                           label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition_PL[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition_PL[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right - Phylogenomic Lineage") +
        theme.my.own)
...
```{r}
PCAdata_Hposition_PG <- plotPCA(vsd, intgroup = c("Sample_Name", "phylogroup"), returnData = TRUE)
percentVar_Hposition_PG <- percentvar_calculation(PCAdata_Hposition_PG)
(PCAplot_PG <- ggplot(PCAdata_Hposition_PG,
                     aes(x = PC1, y = PC2,
                         color = phylogroup,
                         label= Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition_PG[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition_PG[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right - Phylogroup") +
        theme.my.own)
...
```{r}
PCAdata_Hposition_IT <- plotPCA(vsd, intgroup = c("Sample_Name", "Intimin_Type"),
                                returnData = TRUE)
head(PCAdata_Hposition_IT)
percentVar_Hposition_IT <- round(100 * attr(PCAdata_Hposition_IT, "percentVar"), digits = 4)
(PCAplot_IT <- ggplot(PCAdata_Hposition_IT,
                     aes(x = PC1, y = PC2,
                         color = Intimin_Type,
                         label = Sample_Name)) +
        xlab(paste0("PC1: ", percentVar_Hposition_IT[1], "% variance")) +
        ylab(paste0("PC2: ", percentVar_Hposition_IT[2], "% variance")) +
        ggtitle("PCA Plot - Left vs Right - Intimin type") +
        theme.my.own)
...
```{r}
rld <- rlog(dds_Hposition, blind = FALSE)
head(assay(rld), 3)
```
...
(Hclust_plot <- pheatmap(rld_cor, 
                        ...
                        #filename = '~/R/Alina_RNAseq/Hposition/Hclust_plot.tiff'
                       ))
#Hclust_plot
```
...
```{r}
dds1_Hposition <- DESeq(dds_Hposition)
#str(dds1)
```
...
```{r}
res_Hposition <- results(dds1_Hposition, contrast = c("HPosition", "Right", "Left"))
head(res_Hposition, 30)
```

```{r}
summary(res_Hposition)
```
...
```{r}
dds2_Hposition <- DESeq(dds_Hposition, minReplicatesForReplace = Inf)
res2_Hposition <- results(
  dds2_Hposition,
  cooksCutoff = FALSE,
...
head(res2_Hposition, 30)
```

## Results

```{r}
res2df_Hposition <- as.data.frame(res2_Hposition) # convert the results table to a df
head(res2df_Hposition)
```

## MA Plot

```{r}
plotMA_res2_Hposition <- plotMA(res2_Hposition, ylim = c(-2, 2))
```

### Histogram of p-values

```{r}
hist(res2_Hposition$pvalue, breaks = 50, col = "grey50", border = "blue")
```

Further Filtering: baseMean \> 1

```{r}
hist(res2_Hposition$pvalue[res2_Hposition$baseMean > 10],
...
```{r}
res2df_Hposition <- tibble::rownames_to_column(res2df_Hposition, "symbol")
gn <- res2df_Hposition$symbol
...
res2df_Hposition$entrez <- entrez
res2df_Hposition$ensemblID <- ensembl_id
```

Omit NA values from symbol and respective rows!

```{r}
res3df_Hposition <- res2df_Hposition %>% filter(!is.na(symbol) & !is.na(entrez))
nrow(res3df_Hposition)
```

## Saving the Results

```{r}
resOrdered_Hposition <- res3df_Hposition[order(res3df_Hposition$pvalue),]
head(resOrdered_Hposition)
write.csv(as.data.frame(resOrdered_Hposition),
          file = "~/R/Alina_RNAseq/Hposition/results_DGE_ER.csv")
```
...
```{r}
select <- order(rowMeans(counts(dds2_Hposition,normalized = FALSE)),decreasing = TRUE)[1:20]
df <- as.data.frame(colData(dds2_Hposition)[,c("HPosition","Sample_Name")])
...
```{r}
ntd <- normTransform(dds2_Hposition)
...
```{r fig.height= 10, fig.width= 10}
p1 <- EnhancedVolcano(res3df_Hposition,
    lab = res3df_Hposition$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
...
ggsave(filename = '~/R/Alina_RNAseq/Hposition/Volcano1_L2fcVsPvalue.png',
       plot = volcano1,
       dpi = 300,
       width = 5,
       height = 5,
...
```{r fig.height= 10, fig.width= 10}
p2 <- EnhancedVolcano(res3df_Hposition,
    lab = res3df_Hposition$symbol,
    x = 'log2FoldChange',
    y = 'padj',
    xlab = bquote(~Log[2]~'FoldChange'),
...
ggsave(filename = '~/R/Alina_RNAseq/Hposition/Volcano2_L2fcVsPadj.png',
       plot = volcano2,
       dpi = 300,
       width = 10,
       height = 10,
...
```{r}
res3df_Hposition$diffexpressed <- "NS"
# if log2Foldchange > 1.0 and pvalue < 0.05, set as "UP"
res3df_Hposition$diffexpressed[res3df_Hposition$log2FoldChange > 1.0 & res3df_Hposition$pvalue < 0.05] <- "UP"
# if log2Foldchange < -1.0 and pvalue < 0.05, set as "DOWN"
res3df_Hposition$diffexpressed[res3df_Hposition$log2FoldChange < -1.0 & res3df_Hposition$pvalue < 0.05] <- "DOWN"
# Create a new column "delabel" to de, that will contain the name of genes differentially expressed (NA in case they are not)
res3df_Hposition$delabel <- NA
res3df_Hposition$delabel[res3df_Hposition$diffexpressed != "NS"] <- res3df_Hposition$symbol[res3df_Hposition$diffexpressed != "NS"]
```

### Arrive at relevant genes by imposing thresholds.

```{r}
sigsdf_Hposition <- res3df_Hposition[(abs(res3df_Hposition$log2FoldChange) > 1) & (res3df_Hposition$pvalue < 0.05),]
nrow(sigsdf_Hposition)
```
### Gender Genes Removed from Table

```{r}
sigsdf_Hposition <- filter(sigsdf_Hposition,
                          symbol != "Xist",
                          symbol != "Jpx", 
                          symbol != "Ftx", 
                          symbol != "Tsx",
                          symbol != "Cnbp2" )
nrow(sigsdf_Hposition)
#head(sigsdf_Hposition)
```

Therefore, gender genes werent so much in play in terms of significance!

```{r}
write.csv(sigsdf_Hposition ,"~/R/Alina_RNAseq/Hposition/SignificantGenes_ER.csv")
```

### Number of Genes from different strains that are contributing to UP/DOWN regulation.

```{r}
sigsdf_Hposition_UP <- sigsdf_Hposition[(sigsdf_Hposition$log2FoldChange) > 1, ] #UP Regulation Table
sigsdf_Hposition_DOWN <- sigsdf_Hposition[(sigsdf_Hposition$log2FoldChange) < -1, ] #DOWN Regulation Table
nrow(sigsdf_Hposition_UP)
nrow(sigsdf_Hposition_DOWN)
```

### Determine Top20 UP Genes and Top20 DOWN Genes

```{r}
UpGene_Hposition <- sigsdf_Hposition_UP[order(-sigsdf_Hposition_UP$log2FoldChange,decreasing = TRUE), ]$symbol
DownGene_Hposition <- sigsdf_Hposition_DOWN[order(-sigsdf_Hposition_DOWN$log2FoldChange,decreasing = TRUE), ]$symbol
DE_Genes_table_Hposition <- as.data.frame(cbind(UpGene_Hposition,DownGene_Hposition)) # sorted based on highest Log2FC value
```

```{r}
write.csv(UpGene_Hposition ,"~/R/Alina_RNAseq/Hposition/UpGene_Hposition.csv")
write.csv(DownGene_Hposition ,"~/R/Alina_RNAseq/Hposition/DownGene_Hposition.csv")
```
...
sigs2df_Hposition <- res2df_Hposition[(abs(res2df_Hposition$log2FoldChange) > 1) &  (res2df_Hposition$pvalue < 0.05),]
```
...
```{r}
#mat <- counts(dds2_Hposition, normalized = TRUE)[rownames(sigsdf_Hposition),]
mat <- counts(dds2_Hposition, normalized = TRUE)[(sigsdf_Hposition$symbol),]
...
```{r}
mat2 <- counts(dds2_Hposition, normalized = TRUE)[(sigsdf_Hposition$symbol),]
...
newHP <- Heatmap(mat2.zs, 
        ...
        #width = unit(8, "in"), 
        #height = unit(8, "in")
        row_labels = sigs2df_Hposition[rownames(mat2.zs),]$symbol
        )
newHP
```
...
LongHeatMap_Allgenes <- Heatmap(mat2.zs, 
        ...
        cluster_rows = TRUE, 
        column_labels = colnames(mat2.zs),
        row_labels = sigs2df_Hposition$symbol,
        name = 'All DE Genes',
        show_row_names = TRUE, 
        use_raster = TRUE, 
        raster_quality = 5,
...
```{r}
sigs1df_Hposition <- res2df_Hposition[(res2df_Hposition$baseMean > 500) & (abs(res2df_Hposition$log2FoldChange) > 1) & (res2df_Hposition$pvalue < 0.05),]
mat1 <- counts(dds2_Hposition, normalized = TRUE)[(sigs1df_Hposition$symbol),]
...
Heatmap(mat1.zs,
        ...
        name = 'DE Genes',
        row_labels = sigs1df_Hposition$symbol)
```
...
```{r}
UPgene_ENS_ID <- (sigsdf_Hposition_UP$ensemblID)
GO_UPRegResults_Hposition <- enrichGO(gene = UPgene_ENS_ID,
                                       OrgDb = "org.Mm.eg.db",
                                       keyType = "ENSEMBL",
                                       ont = "BP",
                                       pAdjustMethod = "BH",
...
```{r}
GO_UpRegdf_Hposition <- as.data.frame(GO_UPRegResults_Hposition)
head(GO_UpRegdf_Hposition)
```

```{r}
write.csv(GO_UpRegdf_Hposition ,"~/R/Alina_RNAseq/Hposition/GO_UpReg_results_Hposition.csv")
```

```{r fig.height=10, fig.width=10}
GO_UPReg_Barplot_Hposition <- plot(barplot(GO_UPRegResults_Hposition, showCategory = 22))
saveplot(GO_UPReg_Barplot_Hposition, "GO_UPReg_Barplot_Hposition")
```

```{r fig.height=10, fig.width=10}
GO_UPReg_Dotplot_Hposition <- plot(dotplot(GO_UPRegResults_Hposition, showCategory = 23))
saveplot(GO_UPReg_Dotplot_Hposition, "GO_UPReg_Dotplot_Hposition")
```

```{r fig.height=10, fig.width=12}
GO_UPReg_Cnetplot_Hposition <- plot(cnetplot(GO_UPRegResults_Hposition, showCategory = 13))
saveplot(GO_UPReg_Cnetplot_Hposition, "GO_UPReg_Cnetplot_Hposition")
```

### Upset Plot

```{r fig.height= 8, fig.width=12}
GO_UPReg_Upsetplot <- plot( upsetplot(GO_UPRegResults_Hposition))
...
GO_UPReg_Heatplot <- plot( heatplot(GO_UPRegResults_Hposition))
...
edox2 <- pairwise_termsim(GO_UPRegResults_Hposition)
...
```{r}
DOWNgene_ENS_ID <- (sigsdf_Hposition_DOWN$ensemblID)
GO_DOWNRegResults_Hposition <- enrichGO(gene = DOWNgene_ENS_ID,
                       OrgDb = "org.Mm.eg.db",
                       keyType = "ENSEMBL",
                       ont = "BP",
                       pAdjustMethod = "BH",
...
GO_DOWNRegdf_Hposition <- as.data.frame(GO_DOWNRegResults_Hposition)
head(GO_DOWNRegdf_Hposition)
```

```{r}
write.csv(GO_DOWNRegdf_Hposition,
          "~/R/Alina_RNAseq/Hposition/GO_DOWNReg_results_Epithelial.csv")
```

```{r}
GO_DOWNReg_Barplot_Hposition <- plot(barplot(GO_DOWNRegResults_Hposition))
saveplot(GO_DOWNReg_Barplot_Hposition, "GO_DOWNReg_Barplot_Hposition")
```

```{r}
GO_DOWNReg_Dotplot_Hposition <- plot(dotplot(GO_DOWNRegResults_Hposition))

saveplot(GO_DOWNReg_Dotplot_Hposition, "GO_DOWNReg_Dotplot_Hposition")
```

```{r }
GO_DOWNReg_Cnetplot_Hposition <- plot(cnetplot(GO_DOWNRegResults_Hposition,
                                                showCategory = 13,
                                                circular = FALSE,
                                                colorEdge = TRUE
                                                ))

saveplot(GO_DOWNReg_Cnetplot_Hposition, "GO_DOWNReg_Cnetplot_Hposition")
```
...
```{r}
GO_DOWNReg_Upsetplot <- plot( upsetplot(GO_DOWNRegResults_Hposition))
...
```{r}
GO_DOWNReg_Heatplot <- plot(heatplot(GO_DOWNRegResults_Hposition))
...
```{r}
# edox1 <- pairwise_termsim(GO_DOWNRegResults_Hposition)
...
sigsdf_Hposition_unique <- unique(dplyr::filter(sigsdf_Hposition, entrez != "NA"))
```

extract and name the fold changes

```{r}
foldchanges <-  sigsdf_Hposition$log2FoldChange
names(foldchanges) <-  sigsdf_Hposition$entrez
...
# 
# write.csv(gseaKEGG_results, "~/R/Alina_RNAseq/Hposition/gseaOE_kegg.csv")
```
...
```{r}
selectUPGenes_entrezid = unique(sigsdf_Hposition_UP['entrez'] )
selectDOWNGenes_entrezid = unique(sigsdf_Hposition_DOWN['entrez'] )
UniverseGenes = unique(sigsdf_Hposition$entrez)
