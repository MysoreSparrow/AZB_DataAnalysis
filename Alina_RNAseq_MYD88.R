...
here::i_am(path = "Alina_RNAseq_MYD88.R")
paste0(here())
...
Comparison <- "MyD88_InfectedVsControl"
...
sample_ID <- read.csv(file.path(here(), "/MyD88_Samples.csv"))
condition <- c(
...
coldata[coldata == "476_R1"] <- "T_1"
coldata[coldata == "476_R2"] <- "T_2"
coldata[coldata == "757_R1"] <- "L57_1"
coldata[coldata == "757_R2"] <- "L57_2"
coldata[coldata == "766_R1"] <- "L66_1"
coldata[coldata == "766_R2"] <- "L66_2"
coldata[coldata == "768_R1"] <- "A68_1"
coldata[coldata == "768_R2"] <- "A68_2"
coldata[coldata == "Ctrl3_R1"] <- "C1"
coldata[coldata == "Ctrl4_R2"] <- "C2"
# convert column1 with sample names to row.names of coldata
rownames(coldata) <- coldata$Sample_Name
write.csv(coldata, file.path(here(), "coldata_MyD88.csv"))

# Lets Deal with the Countmatrix
# Readin  countsmatrix

countsmatrix <- as.data.frame(read.csv(file.path(here(), "/MyD88_groupedcounts.csv")))
...
write.csv(countsmatrix, file.path(here(), "MyD88_genemappedcounts.csv"))
...
saveplot <- function(plot, plotname) {
  ...
  extension <- ".jpeg"
  ggsave(
    filename = file.path(Comparison_path, paste(plotname, glue("_{Comparison}_"), extension), sep = ""),
    plot = plot, dpi = 400, width = 12, height = 12, units = "in"
  )
  # dev.off()
...
color_values <- c(
  "orange", "orange",  "black", "black", "orange", "orange", "orange", "orange", "blue", "blue")
...
theme.my.own <- list(
  ...
  scale_y_continuous( # limits = c(-15, 20),
    breaks = seq(-15, 20, 5),
    sec.axis = sec_axis(~ . * 1, labels = NULL)
  ), # breaks = NULL
...
(Genes_Biplot <- Genes_Biplot + theme(
...
(Genes_Biplot34 <- fviz_pca_biplot(res.pca,
  axes = c(3, 4),
...
switch(Comparison,
  ...
  "BL6_ER_HighInducerVsLowInducer" = {
    res <- results(dds,
      cooksCutoff = FALSE,
      independentFiltering = FALSE,
      contrast = c("Epithelial_response", "HighInducer", "LowInducer")
    )
  },
  # Epithelial_response
  "BL6_MC_HighVsLow" = {
...
write.csv(significantgenes_df,
          file = file.path(Comparison_path,
...
sigs1df <- resdf[(resdf$baseMean > 10) & (abs(resdf$log2FoldChange) > 2) & (resdf$pvalue < 0.05), ]
...
jpeg(
  file = file.path(Comparison_path, glue("/DEGenes_heatmap3_{Comparison}.jpeg")),
...
GO_UPRegResults <- enrichGO(
  gene = UPgene_ENS_ID, OrgDb = "org.Mm.eg.db", keyType = "ENSEMBL",
...
GO_DOWNRegResults <- enrichGO(
  gene = DOWNgene_ENS_ID, OrgDb = "org.Mm.eg.db", keyType = "ENSEMBL", ont = "BP",
...
run_individualDESeq <- function(namelist, cd, cm) {
  ...
  for (i in 1:8) {
    ...
    cm_input <- cm[, c(i, 9:10)]
    coldata_input <- cd[c(i, 9:10), ]
...
l2fctable <- table %>% select(
  "symbol", "log2FoldChange_T_1", "log2FoldChange_T_2", "log2FoldChange_L57_1",
  "log2FoldChange_L57_2", "log2FoldChange_L66_1", "log2FoldChange_L66_2",
  "log2FoldChange_A68_1", "log2FoldChange_A68_2")
